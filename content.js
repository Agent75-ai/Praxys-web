// Praxys Web — single-source renderer
(function(){
  ...
  const PHOTOS = {
    hero: 'https://images.unsplash.com/photo-1758873268998-2f77c2d38862?auto=format&fit=crop&w=1800&q=74',
    ...
    scenarios: 'https://images.unsplash.com/photo-1559136555-9303baea8ebd?auto=format&fit=crop&w=900&q=72',
  ...
  const CASES = {
    'combined-risk-diagnosis': {
      service:'combined-risk-diagnosis', photo:PHOTOS.collab,
    ...
    'decision-scenario-assessment': {
      service:'decision-scenario-assessment', photo:PHOTOS.scenarios,
  ...
  function renderHero(l){
    ...
    if(sub) sub.textContent = l === 'en' ? 'We analyze complex systems to understand relationships, dependencies, and combined risk effects on continuity, availability, safety, and business objectives.' : 'Analizamos sistemas complejos para comprender relaciones, dependencias y efectos combinados del riesgo sobre continuidad, disponibilidad, seguridad y objetivos del negocio.';
  ...
  function articleField(a, l, esKey, enKey){ return (l==='en' ? (a[enKey] || a[esKey]) : (a[esKey] || a[enKey])) || ''; }
  function renderPapers(l){
    ...
    const grid = sec && sec.querySelector('.praxys-grid'); if(!grid || !wrap) return;
    let controls = sec.querySelector('.px-paper-controls');
    if(!controls){
      controls = document.createElement('div');
      controls.className = 'px-paper-controls';
      controls.innerHTML = `<button type="button" class="px-paper-arrow" data-paper-prev aria-label="${l==='en'?'Previous publication':'Publicación anterior'}">←</button><span class="px-paper-count"><b>1</b> / <em>1</em></span><button type="button" class="px-paper-arrow" data-paper-next aria-label="${l==='en'?'Next publication':'Publicación siguiente'}">→</button>`;
      wrap.insertBefore(controls, grid);
    }
    controls.querySelector('[data-paper-prev]')?.setAttribute('aria-label', l==='en'?'Previous publication':'Publicación anterior');
    controls.querySelector('[data-paper-next]')?.setAttribute('aria-label', l==='en'?'Next publication':'Publicación siguiente');
    grid.className = 'praxys-grid px-paper-reel';
    const source = articles.length ? articles : [];
    grid.innerHTML = source.length ? source.map(a=>{
      const title = articleField(a,l,'title','title_en');
      const desc = articleField(a,l,'desc','desc_en');
      const type = articleField(a,l,'type','type_en') || 'Paper';
      const href = a.link || a.url || '#';
      return `<article class="px-card px-paper"><div class="px-body"><span class="px-kicker">${esc(type)} · ${esc(a.year||'')}</span><h3>${esc(title)}</h3><p>${esc(desc)}</p><p class="px-meta">${esc(a.author||'')}</p>${href && href !== '#'?`<a class="px-link" href="${esc(href)}" target="_blank" rel="noopener">${l==='en'?'Open publication':'Abrir publicación'}</a>`:''}</div></article>`;
    }).join('') : `<article class="px-card px-paper"><div class="px-body"><span class="px-kicker">${l==='en'?'Publications':'Publicaciones'}</span><h3>${l==='en'?'Publications loading':'Publicaciones en carga'}</h3><p>${l==='en'?'The publication reel will appear here when content.json is loaded.':'El reel de publicaciones aparecerá aquí cuando cargue content.json.'}</p></div></article>`;
    setupPaperReel(sec);
  }

  function setupPaperReel(sec){
    const track = sec.querySelector('.px-paper-reel');
    const prev = sec.querySelector('[data-paper-prev]');
    const next = sec.querySelector('[data-paper-next]');
    const current = sec.querySelector('.px-paper-count b');
    const total = sec.querySelector('.px-paper-count em');
    ...
    const nearest = () => {
      ...
      let best = 0, delta = Math.abs(cs[0].offsetLeft - track.scrollLeft);
      cs.forEach((c,i)=>{ const d=Math.abs(c.offsetLeft-track.scrollLeft); if(d<delta){delta=d; best=i;} });
      return best;
    };
    const update = () => { current.textContent = String(Math.min(pages(), Math.floor(nearest()/perView())+1)); total.textContent = String(pages()); };
    const move = dir => { const cs=cards(); if(!cs.length)return; const i=Math.max(0, Math.min(cs.length-1, nearest()+dir*perView())); track.scrollTo({left:cs[i].offsetLeft, behavior:'smooth'}); setTimeout(update,250); };
    if(!sec.dataset.paperBound){
      prev.addEventListener('click',()=>move(-1));
      next.addEventListener('click',()=>move(1));
      track.addEventListener('scroll',()=>{ clearTimeout(track._pxTimer); track._pxTimer=setTimeout(update,80); }, {passive:true});
      window.addEventListener('resize', update);
      sec.dataset.paperBound = '1';
    }
    update();
  }
